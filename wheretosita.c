...
// Function checks permutations with incrementing counter
// Recursive case iterates and sees the elements to place in the index. It checks whether the number is 
// already used in the current partially filled array
// Function recursively calls itself
void printPermutations(int *permutation, int index){
    if(index == n){
        numValid += validOrd(permutation);
        return;
    }
...
int main(){
    ...
    // The number of valid orderings is outputted
    printf("%d\n", numValid);
    return 0;
}